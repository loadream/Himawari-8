...
def get_latest_timestamp():
    ...
    ts_list[-1] = "000" # 将最后一位改为 '0'，不是 '000'
    return "".join(ts_list)
...
def process_and_clean_tiles(timestamp, output_file):
    ...
    if success:
        ...
        # ============================================
        # 3. [新增] 拷贝并覆盖为 last.jpg
        # ============================================
        last_image_path = os.path.join(SAVE_DIR, "last.jpg")
        try:
            # 使用 shutil.copy2 拷贝文件（包括元数据），并覆盖已存在的文件
            shutil.copy2(output_file, last_image_path)
            print("已更新 last.jpg 为最新图片：", last_image_path)
        except Exception as e:
            print(f"拷贝 last.jpg 失败: {e}")
